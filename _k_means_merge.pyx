# Some functions in this module are modified from scikit-learn.
...
def lloyd_iter_chunked_dense(
        ...
        const floating[:, ::1] given_centers,               # IN
        const floating[::1] given_weights_in_clusters,      # IN
    ...
    if update_centers:
        ...
        memcpy(&centers_new[0, 0],
               &given_centers[0, 0],
               n_clusters * n_features * sizeof(floating))
        memcpy(&weight_in_clusters[0],
               &given_weights_in_clusters[0],
               n_clusters * sizeof(floating))
        omp_init_lock(&lock)
        # for j in range(n_clusters):
        #     weight_in_clusters[j] = given_weights_in_clusters[j]
        #     for k in range(n_features):
        #         centers_new[j, k] = given_centers[j, k]
    ...
    if update_centers:
        ...
        _average_centers(centers_new, weight_in_clusters) # TBU
...
cdef void _update_chunk_dense(
    ...
    for i in range(n_samples):
        ...
        if update_centers:
            ...
            for k in range(n_features):
                centers_new[label * n_features + k] += X[i, k] * sample_weight[i] ## TBU
